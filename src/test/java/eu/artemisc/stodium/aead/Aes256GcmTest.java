...
import eu.artemisc.stodium.codecs.Codec;
...
public class Aes256GcmTest {
    ...
    public void tv()
        ...
        long found_ciphertext_len;
        long found_mac_len;
        long found_message_len;
        int  ad_len;
        int  ciphertext_len;
        int  detached_ciphertext_len;
        int  message_len;

        codec = Codec.hex();
        aead = AEAD.aesInstance();
        if (aead == null) {
            Assert.fail("expected Aes");
            return;
        }

        key = ByteBuffer.allocateDirect(aead.keyBytes());
        nonce = ByteBuffer.allocateDirect(aead.npubBytes());
        mac = ByteBuffer.allocateDirect(aead.aBytes());

        for (int i = 0; i < tests.length; ++i) {
            Assert.assertEquals(keyHex(i).length(), 2 * aead.keyBytes());
            codec.decode(key, keyHex(i));

            Assert.assertEquals(nonceHex(i).length(), 2 * aead.npubBytes());
            codec.decode(nonce, nonceHex(i));

            message_len = messageHex(i).length() / 2;
            message = ByteBuffer.allocateDirect(message_len);
            codec.decode(message, messageHex(i));

            ad_len = adHex(i).length() / 2;
            ad = ByteBuffer.allocateDirect(ad_len);
            codec.decode(ad, adHex(i));

            ciphertext_len = message_len + aead.aBytes();
            detached_ciphertext_len = message_len;
            expected_ciphertext = ByteBuffer.allocateDirect(ciphertext_len);
            Assert.assertEquals(cipherHex(i).length(), 2 * message_len);
            codec.decode(expected_ciphertext, cipherHex(i));

            Assert.assertEquals(macHex(i).length(), 2 * aead.aBytes());
            expected_mac = ((ByteBuffer) expected_ciphertext.slice().position(message_len)).slice();
            codec.decode(expected_mac, macHex(i));
            ciphertext = ByteBuffer.allocateDirect(ciphertext_len);
            detached_ciphertext = ByteBuffer.allocateDirect(detached_ciphertext_len);


            aead.encryptDetached(detached_ciphertext, mac, message, ad, nonce, key);
            Assert.assertEquals(mac.remaining(), aead.aBytes());

            if (!Stodium.isEqual(detached_ciphertext, (ByteBuffer) expected_ciphertext.slice().limit(detached_ciphertext.remaining())) ||
                    !Stodium.isEqual(mac, (ByteBuffer) expected_ciphertext.slice().position(detached_ciphertext.remaining()))) {
//                Assert.fail();
//                Assert.fail(String.format(
//                        "Detached encryption of text vector %d failed\ncomputed: [%s]",
//                        i, codec.encode(detached_ciphertext)));
            }

            aead.encrypt(ciphertext, message, ad, nonce, key);

            Assert.assertEquals(ciphertext.remaining(), ciphertext_len);
            if (!Stodium.isEqual(ciphertext, expected_ciphertext)) {
                Assert.fail(String.format(
                        "Encryption of text vector %d failed\ncomputed: [%s]",
                        i, codec.encode(ciphertext)));
            }
...
//            }
    ...
    @NotNull
    private static String keyHex(final int i) {
        return tests[i][0];
    }

    @NotNull
    private static String nonceHex(final int i) {
        return tests[i][1];
    }

    @NotNull
    private static String messageHex(final int i) {
        return tests[i][2];
    }

    @NotNull
    private static String adHex(final int i) {
        return tests[i][3];
    }

    @NotNull
    private static String cipherHex(final int i) {
        return tests[i][4];
    }

    @NotNull
    private static String macHex(final int i) {
        return tests[i][5];
    }

    private static final @NotNull String[][] tests = new String[][]{
            {
